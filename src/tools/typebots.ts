...
export function registerTypebotTools(server: McpServer) {
  server.registerTool(
    "list_typebots",
    {
      title: "List Typebots",
      description:
        "List all typebots in a workspace. Optionally filter by folder. Returns id, name, icon, publishedTypebotId and accessRight for each bot.",
      inputSchema: {
    ...
    async ({ workspace_id, folder_id }) => {
      try {
        ...
        return toolError(`Failed to list typebots: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "get_typebot",
    {
      title: "Get Typebot",
      description:
        "Get complete details of a typebot by its ID. Returns the full bot configuration including groups, edges, variables, theme, settings.",
      inputSchema: {
    ...
    async ({ typebot_id, migrate_to_latest_version }) => {
      try {
        ...
        return toolError(`Failed to get typebot: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ workspace_id, name, folder_id, icon }) => {
      try {
        const typebot: Record<string, unknown> = { name };
        if (folder_id) typebot.folderId = folder_id;
        if (icon) typebot.icon = icon;
        ...
        return toolError(`Failed to create typebot: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "update_typebot",
    {
      ...
      description:
        "Update an existing typebot. Pass any subset of typebot fields to update (name, icon, theme, settings, groups, edges, variables, etc.).",
      inputSchema: {
  ...
  server.registerTool(
    ...
    async ({ typebot_id }) => {
      try {
        ...
        return toolError(`Failed to delete typebot: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ typebot_id }) => {
      try {
        ...
        return toolError(`Failed to publish typebot: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ typebot_id }) => {
      try {
        ...
        return toolError(`Failed to unpublish typebot: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "get_published_typebot",
    {
      ...
      description:
        "Get the published version of a typebot. Returns the live configuration with groups, edges, variables, theme and settings.",
      inputSchema: {
        typebot_id: z.string().describe("Typebot ID"),
        migrate_to_latest_version: z
          .boolean()
    ...
    async ({ typebot_id, migrate_to_latest_version }) => {
      try {
        const data = await apiRequest(
          `/v1/typebots/${typebot_id}/publishedTypebot`,
          "GET",
          undefined,
          {
            migrateToLatestVersion: migrate_to_latest_version?.toString(),
        ...
        return toolError(`Failed to get published typebot: ${(error as Error).message}`);
      }
    },
  );
}
