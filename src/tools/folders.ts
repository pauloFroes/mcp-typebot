...
export function registerFolderTools(server: McpServer) {
  server.registerTool(
    "list_folders",
    {
      title: "List Folders",
      description:
        "List folders in a workspace. Optionally filter by parent folder to navigate the folder tree.",
      inputSchema: {
  ...
  server.registerTool(
    ...
    async ({ workspace_id, folder_name, parent_folder_id }) => {
      try {
        const body: Record<string, unknown> = { workspaceId: workspace_id };
        if (folder_name) body.folderName = folder_name;
        if (parent_folder_id) body.parentFolderId = parent_folder_id;
  ...
  server.registerTool(
    ...
    async ({ folder_id, workspace_id, name, parent_folder_id }) => {
      try {
        ...
        if (parent_folder_id !== undefined) folder.parentFolderId = parent_folder_id;
  ...
  server.registerTool(
    "delete_folder",
    {
      title: "Delete Folder",
      description:
        "Delete a folder and its contents. This action cannot be undone.",
      inputSchema: {
