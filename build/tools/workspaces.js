...
export function registerWorkspaceTools(server) {
    server.registerTool("list_workspaces", {
        ...
        description: "List all workspaces the authenticated user has access to. Returns id, name, icon and plan for each workspace.",
        inputSchema: {},
        annotations: {
            readOnlyHint: true,
        ...
        try {
            ...
            return toolError(`Failed to list workspaces: ${error.message}`);
        }
    });
    server.registerTool("get_workspace", {
        title: "Get Workspace",
        description: "Get detailed information about a workspace including plan, limits, suspension status, and verification.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to get workspace: ${error.message}`);
        }
    });
    server.registerTool("create_workspace", {
        ...
        try {
            ...
            return toolError(`Failed to create workspace: ${error.message}`);
        }
    });
    server.registerTool("update_workspace", {
        ...
        try {
            ...
            return toolError(`Failed to update workspace: ${error.message}`);
        }
    });
    server.registerTool("delete_workspace", {
        title: "Delete Workspace",
        description: "Permanently delete a workspace and all its typebots. This action cannot be undone.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to delete workspace: ${error.message}`);
        }
    });
    server.registerTool("list_workspace_members", {
        title: "List Workspace Members",
        description: "List all members of a workspace. Returns user info (name, email, image) and role (ADMIN, MEMBER, GUEST).",
        inputSchema: {
            workspace_id: z.string().describe("Workspace ID"),
        },
        annotations: {
        ...
        try {
            ...
            return toolError(`Failed to list workspace members: ${error.message}`);
        }
    });
}
